...
def checkFlow(data, code):
    ...
    if os.path.exists( code ):
       ...
       print(trackTaint(data, var_df, call_df, func_def_df, func_var_df)) # Completed by Mary Mitchell for Workshop2




# Completed by Mary Mitchell for Workshop2
def trackTaint(data, var_df, call_df, func_def_df, func_var_df):
    output = str(data)
    output += "->"
    index = 0
    for i in range(len(var_df.RHS)):
        if data == var_df.RHS[i]:
            index = i
    var = var_df.LHS[index]
    output += str(var)
    output += "->"
    for i in range(len(call_df.ARG_NAME)):
        strg = str(call_df.ARG_NAME[i])
        if var == strg:
           index = i
    parm = call_df.TYPE[index]
    parm_num = ''.join(filter(str.isdigit, parm))
    for i in range(len(func_def_df.TYPE)):
        strg = str(''.join(filter(str.isdigit, func_def_df.TYPE[i])))
        if parm_num == strg:
            index = i
    parm_name = func_def_df.ARG_NAME[index]
    output += str(parm_name)
    output += "->"
    for i in range(len(func_var_df.RHS)):
        strg = str(func_var_df.RHS[i])
        if parm_name in strg:
            index = i
    assgnmt = func_var_df.LHS[index]
    output += str(assgnmt)
    return output
